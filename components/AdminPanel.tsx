...
import { Loader, Save, Shield, AlertTriangle, ExternalLink, RotateCcw, Users, Edit3, Info, Database, CheckSquare } from 'lucide-react';
...
export const AdminPanel: React.FC<AdminPanelProps> = ({ showToast, user }) => {
    const { t } = useTranslation();
    const [users, setUsers] = useState<User[]>([]);
    const [isLoading, setIsLoading] = useState(true);
    const [isFirstAdmin, setIsFirstAdmin] = useState(false);
    const [permissionError, setPermissionError] = useState(false);
    const [activeTab, setActiveTab] = useState<'users' | 'prompts' | 'system-check'>('users');
        ...
        <div className="max-w-5xl mx-auto">
            ...
            <div className="border-b border-gray-200 dark:border-gray-700">
                <nav className="flex space-x-2" aria-label="Tabs">
                    ...
                    />
                    <TabButton 
                        label={t('adminPanel.systemCheckTab')} 
                        icon={<CheckSquare size={16} />} 
                        isActive={activeTab === 'system-check'} 
            ...
            <div className="mt-6">
                ...
                {activeTab === 'system-check' && <SystemCheckPanel user={user} />}
            </div>
        </div>
    );
};
